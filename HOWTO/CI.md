...
## Conditional Branching using otp versions

Conditional branching is discouraged, as it breaks the uniformity between
different maintained branches. But it may be inevitable in cases. Here we
provide a way to add those exceptional cases:

The reusable workflow `reusable-setup.yaml` will be triggered from the branch
that created a pull request, push, or some other condition. This workflow sets
up an output variable, `otp_version` that matches the version of the `OTP_VERSION`
file as follows, and adds a `maint-XX`:
...
As such, no maintainance branch nor `maint` branch that turns into maintainance branch needs to be update
if scripts use branching conditions, such as:

```yaml
- name: Create a release
  if: ${{ inputs.otp_branch != 'maint-26' && inputs.otp_branch != 'maint-27' }} # skip for OTP-26 and 27
  env:
...
- name: Create Release for 26 and 27
  if: ${{ inputs.otp_branch == 'maint-26' || inputs.otp_branch == 'maint-27' }}
  env:
    GH_TOKEN: ${{ secrets.GITHUB_TOKEN }}
    TAG:      ${{ inputs.tag }}
    VSN:      ${{ inputs.vsn }}
