...
-doc "Searches the list of tuples `TupleList` for a tuple whose `N`th element compares equal to `Key`\\. Returns `Tuple` if such a tuple is found, otherwise `false`\\.".
...
-doc "Returns `true` if there is a tuple in `TupleList` whose `N`th element compares equal to `Key`, otherwise `false`\\.".
...
-doc "Searches the list of tuples `TupleList` for a tuple whose `N`th element compares equal to `Key`\\. Returns `{value, Tuple}` if such a tuple is found, otherwise `false`\\.

### Note:

  This function is retained for backward compatibility\\. Function [`keyfind/3`](https://erlang.org/doc/man/lists.html#keyfind-3) is usually more convenient\\.".
...
-doc "Returns a list with the elements in `List1` in reverse order, with tail `Tail` appended\\.
...
-doc "Returns a new list `List3` that is a copy of `List1`, subjected to the following procedure: for each element in `List2`, its first occurrence in `List1` is deleted\\.
...
-doc "Returns the `N`th element of `List`\\.
...
-doc "Returns the `N`th tail of `List`, that is, the sublist of `List` starting at `N+1` and continuing up to the end of the list\\.
...
-doc "Drops the last element of a `List`\\. The list is to be non\\-empty, otherwise the function crashes with a `function_clause`\\.".
...
-doc "Returns a sequence of integers that starts with `From` and contains the successive results of adding `Incr` to the previous element, until `To` is reached or passed \\(in the latter case, `To` is not an element of the sequence\\)\\. `Incr` defaults to 1\\.
...
-doc "Returns a list containing `N` copies of term `Elem`\\.
...
-doc "Returns the first element of `List` that compares less than or equal to all other elements of `List`\\.".
...
-doc "Returns the first element of `List` that compares greater than or equal to all other elements of `List`\\.".
...
-doc "Returns the sublist of `List1` starting at `Start` and with \\(maximum\\) `Len` elements\\. It is not an error for `Start+Len` to exceed the length of the list\\.
...
-doc "Returns the sublist of `List1` starting at position 1 and with \\(maximum\\) `Len` elements\\. It is not an error for `Len` to exceed the length of the list, in that case the whole list is returned\\.".
...
-doc "Returns a copy of `List1` where the first element matching `Elem` is deleted, if there is such an element\\.".
...
-doc "\"Zips\" two lists into one list of two\\-tuples, where the first element of each tuple is taken from the first list and the second element is taken from the corresponding element in the second list\\.

The `How` parameter specifies the behavior if the given lists are of different lengths\\.

* **`fail`**  
  The call will fail if the given lists are not of equal length\\. This is the default\\.

* **`trim`**  
  Surplus elements from the longer list will be ignored\\.
  *Examples:*
...
* **`{pad, Defaults}`**  
  The shorter list will be padded to the length of the longer list, using the respective elements from the given `Defaults` tuple\\.
  *Examples:*
...
-doc "\"Unzips\" a list of two\\-tuples into two lists, where the first list contains the first element of each tuple, and the second list contains the second element of each tuple\\.".
...
-doc "\"Zips\" three lists into one list of three\\-tuples, where the first element of each tuple is taken from the first list, the second element is taken from the corresponding element in the second list, and the third element is taken from the corresponding element in the third list\\.

For a description of the `How` parameter, see [`zip/3`](https://erlang.org/doc/man/lists.html#zip-3)\\.".
...
-doc "\"Unzips\" a list of three\\-tuples into three lists, where the first list contains the first element of each tuple, the second list contains the second element of each tuple, and the third list contains the third element of each tuple\\.".
...
-doc "Combines the elements of two lists into one list\\. For each pair `X, Y` of list elements from the two lists, the element in the result list is `Combine(X, Y)`\\.
...
-doc "Combines the elements of three lists into one list\\. For each triple `X, Y, Z` of list elements from the three lists, the element in the result list is `Combine(X, Y, Z)`\\.
...
-doc "Returns the sorted list formed by merging all the sublists of `ListOfLists`\\. All sublists must be sorted before evaluating this function\\. When two elements compare equal, the element from the sublist with the lowest position in `ListOfLists` is picked before the other element\\.".
...
-doc "Returns the sorted list formed by merging `List1` and `List2`\\. Both `List1` and `List2` must be sorted before evaluating this function\\. When two elements compare equal, the element from `List1` is picked before the element from `List2`\\.".
...
-doc "Concatenates the text representation of the elements of `Things`\\. The elements of `Things` can be atoms, integers, floats, or strings\\.
...
-doc "Equivalent to `length(flatten(DeepList))`, but more efficient\\.".
...
-doc "Returns a copy of `TupleList1` where the first occurrence of a tuple whose `N`th element compares equal to `Key` is deleted, if there is such a tuple\\.".
...
-doc "Returns a copy of `TupleList1` where the first occurrence of a `T` tuple whose `N`th element compares equal to `Key` is replaced with `NewTuple`, if there is such a tuple `T`\\.".
...
-doc "Searches the list of tuples `TupleList1` for a tuple whose `N`th element compares equal to `Key`\\. Returns `{value, Tuple, TupleList2}` if such a tuple is found, otherwise `false`\\. `TupleList2` is a copy of `TupleList1` where the first occurrence of `Tuple` has been removed\\.".
...
-doc "Returns a copy of `TupleList1` where the first occurrence of a tuple `T` whose `N`th element compares equal to `Key` is replaced with `NewTuple`, if there is such a tuple `T`\\. If there is no such tuple `T`, a copy of `TupleList1` where \\[`NewTuple`\\] has been appended to the end is returned\\.".
...
-doc "Returns a list containing the sorted elements of list `TupleList1`\\. Sorting is performed on the `N`th element of the tuples\\. The sort is stable\\.".
...
-doc "Returns a list containing the sorted elements of list `TupleList1` where all except the first tuple of the tuples comparing equal have been deleted\\. Sorting is performed on the `N`th element of the tuples\\.".
...
-doc "Returns a list of tuples where, for each tuple in `TupleList1`, the `N`th element `Term1` of the tuple has been replaced with the result of calling `Fun(Term1)`\\.
...
-doc "Returns `List1` with each element `H` replaced by a tuple of form `{I, H}` where `I` is the position of `H` in `List1`\\. The enumeration starts with `Index` and increases by `Step` in each step\\.
...
-doc "Returns a list containing the sorted elements of `List1`, according to the [ordering function](https://erlang.org/doc/man/lists.html#ordering_function) `Fun`\\. `Fun(A, B)` is to return `true` if `A` compares less than or equal to `B` in the ordering, otherwise `false`\\.".
...
-doc "Returns the sorted list formed by merging `List1` and `List2`\\. Both `List1` and `List2` must be sorted according to the [ordering function](https://erlang.org/doc/man/lists.html#ordering_function) `Fun` before evaluating this function\\. `Fun(A, B)` is to return `true` if `A` compares less than or equal to `B` in the ordering, otherwise `false`\\. When two elements compare equal, the element from `List1` is picked before the element from `List2`\\.".
...
-doc "Returns a list containing the sorted elements of `List1` where all except the first element of the elements comparing equal according to the [ordering function](https://erlang.org/doc/man/lists.html#ordering_function) `Fun` have been deleted\\. `Fun(A, B)` is to return `true` if `A` compares less than or equal to `B` in the ordering, otherwise `false`\\.".
...
-doc "Returns the sorted list formed by merging `List1` and `List2`\\. Both `List1` and `List2` must be sorted according to the [ordering function](https://erlang.org/doc/man/lists.html#ordering_function) `Fun` and contain no duplicates before evaluating this function\\. `Fun(A, B)` is to return `true` if `A` compares less than or equal to `B` in the ordering, otherwise `false`\\. When two elements compare equal, the element from `List1` is picked and the one from `List2` is deleted\\.".
...
-doc "Returns a list containing the sorted elements of `List1` where all except the first element of the elements comparing equal have been deleted\\.".
...
-doc "Returns the sorted list formed by merging all the sublists of `ListOfLists`\\. All sublists must be sorted and contain no duplicates before evaluating this function\\. When two elements compare equal, the element from the sublist with the lowest position in `ListOfLists` is picked and the other is deleted\\.".
...
-doc "Returns the sorted list formed by merging `List1` and `List2`\\. Both `List1` and `List2` must be sorted and contain no duplicates before evaluating this function\\. When two elements compare equal, the element from `List1` is picked and the one from `List2` is deleted\\.".
...
-doc "Returns `true` if `Pred(Elem)` returns `true` for all elements `Elem` in `List`, otherwise `false`\\. The `Pred` function must return a boolean\\.".
...
-doc "Returns `true` if `Pred(Elem)` returns `true` for at least one element `Elem` in `List`\\. The `Pred` function must return a boolean\\.".
...
-doc "Takes a function from `A`s to `B`s, and a list of `A`s and produces a list of `B`s by applying the function to every element in the list\\. This function is used to obtain the return values\\. The evaluation order depends on the implementation\\.".
...
-doc "Takes a function from `A`s to lists of `B`s, and a list of `A`s \\(`List1`\\) and produces a list of `B`s by applying the function to every element in `List1` and appending the resulting lists\\.
...
-doc "Calls `Fun(Elem, AccIn)` on successive elements `A` of `List`, starting with `AccIn == Acc0`\\. `Fun/2` must return a new accumulator, which is passed to the next call\\. The function returns the final value of the accumulator\\. `Acc0` is returned if the list is empty\\.
...
-doc "Like [`foldl/3`](https://erlang.org/doc/man/lists.html#foldl-3), but the list is traversed from right to left\\.
...
-doc "`List2` is a list of all elements `Elem` in `List1` for which `Pred(Elem)` returns `true`\\. The `Pred` function must return a boolean\\.".
...
-doc "Partitions `List` into two lists, where the first list contains all elements for which `Pred(Elem)` returns `true`, and the second list contains all elements for which `Pred(Elem)` returns `false`\\.
...
For a different way to partition a list, see [`splitwith/2`](https://erlang.org/doc/man/lists.html#splitwith-2)\\.".
...
-doc "Calls `Fun(Elem)` on successive elements `Elem` of `List1`\\. `Fun/1` must return either a Boolean or a tuple `{true, Value}`\\. The function returns the list of elements for which `Fun` returns a new value, where a value of `true` is synonymous with `{true, Elem}`\\.
...
-doc "Calls `Fun(Elem)` for each element `Elem` in `List`\\. This function is used for its side effects and the evaluation order is defined to be the same as the order of the elements in the list\\.".
...
-doc "Combines the operations of [`map/2`](https://erlang.org/doc/man/lists.html#map-2) and [`foldl/3`](https://erlang.org/doc/man/lists.html#foldl-3) into one pass\\.
...
-doc "Combines the operations of [`map/2`](https://erlang.org/doc/man/lists.html#map-2) and [`foldr/3`](https://erlang.org/doc/man/lists.html#foldr-3) into one pass\\.".
...
-doc "Takes elements `Elem` from `List1` while `Pred(Elem)` returns `true`, that is, the function returns the longest prefix of the list for which all elements satisfy the predicate\\. The `Pred` function must return a boolean\\.".
...
-doc "Drops elements `Elem` from `List1` while `Pred(Elem)` returns `true` and returns the remaining list\\. The `Pred` function must return a boolean\\.".
...
-doc "If there is a `Value` in `List` such that `Pred(Value)` returns `true`, returns `{value, Value}` for the first such `Value`, otherwise returns `false`\\. The `Pred` function must return a boolean\\.".
...
-doc "Partitions `List` into two lists according to `Pred`\\. `splitwith/2` behaves as if it is defined as follows:
...
The `Pred` function must return a boolean\\. For a different way to partition a list, see [`partition/2`](https://erlang.org/doc/man/lists.html#partition-2)\\.".
...
-doc "Splits `List1` into `List2` and `List3`\\. `List2` contains the first `N` elements and `List3` the remaining elements \\(the `N`th tail\\)\\.".
...
-doc "Inserts `Sep` between each element in `List1`\\. Has no effect on the empty list and on a singleton list\\. For example:
...
-doc "Returns a list containing the elements of `List1` with duplicated elements removed \\(preserving the order of the elements\\)\\. The first occurrence of each element is kept\\.
...
-doc "Returns a list containing the elements of `List1` without the elements for which `Fun` returned duplicate values \\(preserving the order of the elements\\)\\. The first occurrence of each element is kept\\.
