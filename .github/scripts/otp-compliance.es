                             ...
                             case Purl of
                                 <<"pkg:otp/erlang", _/binary>> ->
                                     case {FixedStatus, AffectedStatus} of
                                         {true, true} ->
                                             throw("Erlang/OTP versions, (e.g.) OTP-26.1 do not support fixed and affected status");
                                         _ ->
                                             [format_vexctl(InitVex, Purl, CVE, Status)]
                                     end;
                                 <<"pkg:otp/", _/binary>> -> % handle OTP Apps, pkg:otp/ssl@4.3.1
                                     FixedRange = case maps:get(~"fixed", Status, <<>>) of
                                                      <<>> ->
                                                          [];
                                                      L when is_list(L) ->
                                                          L
...
test_openvex(_) ->
    ok = test_openvex_branched_otp_tree(),
    ok = test_openvex_branched_otp_tree_idempotent(),
    ok.
